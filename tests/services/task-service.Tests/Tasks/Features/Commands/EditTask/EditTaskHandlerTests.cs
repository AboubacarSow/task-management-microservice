...
namespace task_service.Tests.Tasks.Features.Commands.EditTask;
...
public class EditTaskHandlerTests
{
    ...
    {
        _handler = new EditTaskHandler(_taskRepo.Object, 
            _projectClientMock.Object,_loggerMock.Object);
    }
    ...
    public async Task Should_Update_All_Fields()
    {
        ...
        var group = new List<Guid> { Guid.NewGuid(), userId };
        var project = FakeProjectModelData.BuildProjectModel(projectId, Guid.NewGuid(),
            group.AsEnumerable());
        var task = new TaskItem("old", projectId, userId);
