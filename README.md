...
graph TB
    ...
    RabbitMQ -->|Consume Events| Report
...
### 2.5 Request Flow Through Gateway
...
sequenceDiagram
    ...
    Gateway->>Gateway: GlobalExceptionHandlerMiddleware
    Gateway->>Gateway: CorrelationIdMiddleware (X-Correlation-ID)
    Gateway->>Gateway: Validate JWT signature
    Gateway->>Gateway: Check scope (project_fullpermission)
    Gateway->>ProjectService: PATCH /api/projects/{id} + JWT + X-Correlation-ID
    ProjectService->>MongoDB: GetByIdAsync(id)
    MongoDB-->>ProjectService: Project
    ProjectService->>ProjectService: Check ownership (CreatedByUser == sub)
    ProjectService->>MongoDB: EditAsync(project)
    MongoDB-->>ProjectService: OK
    ProjectService-->>Gateway: 204 NoContent
    Gateway-->>Client: 204 NoContent
```

### 2.6 Token Refresh Flow
...
### 2.10 Dispatcher Middleware Pipeline
...
| `ai-agent-service` | FastAPI + MongoDB + Ollama | AI-powered task suggestions via llama3.2 | ✅ Done |
| `report-service` | .NET 9 + MongoDB | Aggregated reports from RabbitMQ events | 🔄 In Progress |
...
> _Screenshot: Postman POST /login returning JWT access token and refresh token_

### 4.2 JWT Claims — Decoded Token

> _Screenshot: jwt.io showing decoded token with sub, email, given_name, family_name, scope claims_

### 4.3 Project Operations

> _Screenshot: Postman POST /projects returning 201 Created_
> _Screenshot: Postman PATCH /projects/{id} returning 204 NoContent_
> _Screenshot: Postman PATCH /projects/{id}/complete returning 204 NoContent_

### 4.4 Unit Test Results

#### Authentication Service — 17 Tests ✅
> _Screenshot: dotnet test output — all GREEN_
...
> _Screenshot: Grafana k6 dashboard — real-time metrics during load test_

### 4.6 Observability

> _Screenshot: Kibana — structured logs with X-Correlation-ID across services_
> _Screenshot: Grafana — service metrics dashboard (request rate, latency, errors)_
> _Screenshot: Prometheus — targets page showing all services UP_
...
- ✅ Full microservice architecture with 7 independent services
...
- ✅ TDD across all .NET services — 59+ passing tests
